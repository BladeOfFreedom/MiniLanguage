...
import Physics.PlayerClasses.Player;
...
public class PhysicsManager {
	...
	public static void checkPlayer(Player p, CopyOnWriteArrayList<RigidBody> bodies){
		for(RigidBody body: bodies) {
			if(body instanceof Plane2D) {
				Plane2D p1 = (Plane2D) body;

				double distance = Vector2D.dot(Vector2D.subtract(p.position, p1.position), p1.normal);

				if (distance < p.getHurtBoxRadius()) {
					//need the calculations
					double velocity_along_normal = Vector2D.dot(p1.normal, p.velocity);
					...
					//calculate bounce
					double e =  Math.min(p.getE(), p1.getE());

					double totalInverseMass = p.getInverseMass();
					if (totalInverseMass == 0) {
						return; // Two static objects colliding? Do nothing.
					}
					...
					p.velocity.add(Vector2D.scale(Vector2D.scale(p1.normal, j), p.getInverseMass()));

					Vector2D planeToCircle = Vector2D.subtract(p.position, p1.position);

					// 2. Project that vector onto the Normal to find "Altitude"
					double distanceToPlane = Vector2D.dot(planeToCircle, p1.normal);

					// 3. Calculate Overlap (Radius - Altitude)
					double overlap = p.getHurtBoxRadius() - distanceToPlane;
